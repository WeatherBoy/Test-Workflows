...
def split_the_difference_datetime(json_response: Any, response_key: str | None):
    """
    Given a response to a questionnaire loaded from JSON, convert it to a float.

    If the response is a string representing a time (e.g., "23:15"),
    parse it into a datetime object and return it.
    If the response is a string representing a range (e.g., "23:00-23:30"),
    split the range, parse both parts into datetime objects, and return their midpoint.
    If the response is invalid (neither a valid time nor a valid range string),
    raise a ValueError with an appropriate message.

    :param json_response: The response value from the JSON, either a time string or a range string (type is `Any`, but for this data it should be a string).
    :param response_key: Optional key name for error messages.

    :return: The response converted to a datetime object.
    """
    if response_key is None:
        response_key = ""

    if isinstance(json_response, str) and "-" in json_response:
        parts = json_response.split("-")
        low = parts[0].strip()
        high = parts[1].strip()

        time_format = "%H:%M"
        low_dt = datetime.strptime(low, time_format)
        high_dt = datetime.strptime(high, time_format)
        if high_dt < low_dt:
            high_dt += timedelta(days=1)  # wrap around midnight
        mid_dt = low_dt + (high_dt - low_dt) / 2
        datetime_response = mid_dt

    elif isinstance(json_response, str):
        time_format = "%H:%M"
        datetime_response = datetime.strptime(json_response.strip(), time_format)
